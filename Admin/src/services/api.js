...
api.interceptors.request.use(
  (config) => {
    // Get token from localStorage (try all possible locations)
    ...
    // API path check to determine which token to use
    const isAdminEndpoint = config.url.startsWith('/Admin') || 
                            config.url.toLowerCase().includes('admin');
    
    // Try to get token from user or admin object as backup
    try {
      ...
      if (admin) {
        ...
        const adminUserToken = adminData.accessToken;
        
        // Log admin user info
        if (isAdminEndpoint) {
          console.log('Admin data from localStorage:', {
            id: adminData.id,
            role: adminData.role || 'no role defined',
            hasAdminToken: !!adminToken,
            hasAdminUserToken: !!adminUserToken,
            endpoint: config.url
          });
        }
        
        // Update adminToken if we found one in the user object
        if (!adminToken && adminUserToken) {
    ...
    // Choose the appropriate token based on endpoint
    let finalToken = null;
    if (isAdminEndpoint) {
      finalToken = adminToken || token || userToken;
      console.log('Using admin token for endpoint:', config.url);
    } else {
      finalToken = token || userToken || adminToken;
    }
